...
The watcher also bounds inconclusive ordinary workers through `pane_turn_over_age` in `bin/fm-watch.sh`, so changing rendered bytes alone cannot suppress inspection indefinitely.
