...
Bootstrap validates `approval`, `floor`, and present `provider` values only while typed resolution is active; without the key those fields remain unvalidated by bootstrap, preserving its pre-existing verified-harness baseline.
...
Everything after the answer runs in code: the confidence floor, the matched rule's `approval` and `floor`, each candidate's `provider` and `floor`, every applicable account-wide and model/product row from one `quota-axi --json` snapshot, and the rule's selection policy declared above.
Quota-balanced selection uses numeric `spendPriority` argmax over candidates using each candidate's limiting row; a sole eligible candidate unranked only for unknown quota can also clear with a disclosure note, unless its profile floor is unverifiable.
...
The result is one of `clear` (a `profile:` line ready for `fm-spawn.sh`), `ambiguous` (confidence below the floor), `escalate` (an approval-gated rule, unverifiable required floor, no eligible choice, no rankable winner outside the sole-unknown exception, or a genuine quota-balanced tie), or `error` (API, network, malformed response metadata, rendering, or quota-axi failure), and every one of them exits 0.
...
Set `FM_DISPATCH_RESOLVE_LOG` to an append-only JSONL destination to opt into per-invocation evidence (requires `python3`). Its parent directory must already exist; new files are created with mode 0600. Each line records a UTC timestamp, SHA-256 of the brief path exactly as supplied (not its contents), status and reason, matched rule and confidence, API-reported Jev model and input/output token counts, latency in milliseconds, selected profile, known-ineligible skipped candidates with reported reset times, and `intake: clear` or `fallback`. Unavailable scalar fields are null and candidate lists are empty; early exits, including off and configuration errors, are recorded too. Configuration errors use a generic reason and HTTP errors omit response bodies. No brief text, API keys, full response, or quota snapshot is recorded; this is evidence, not quota caching. Protect and rotate this operator-owned file as needed. Logging failures emit a generic warning but never change resolution output or exit status. With the variable unset, no logger runs and resolver behavior is unchanged.
...
The resolver fixes the endpoint at `https://api.typesafe.ai`, model at `jev-latest`, confidence floor at 0.6, and request timeout at 5 seconds.
