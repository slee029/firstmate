...
- `quota-array-dispatch` - load before choosing among a matched crew-dispatch profile array from current quota-axi default TOON, and at review setup before pinning `review_agents.reviewer` for a no-mistakes run, so the reviewer seat is resolved through `bin/fm-reviewer-choose.sh`.
