...
In quota-balanced mode, break genuine evidence ties without array-order or harness bias; array order applies only under preference.
`quota-axi` owns how model or product windows relate to bounding account windows and remains data-only.
Load `quota-array-dispatch` before choosing among a matched profile array; that skill is the single owner of the profile-array selection procedure.
